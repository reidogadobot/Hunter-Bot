	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: '❌ DEU EERO :( TENTE NOVAMENTE! ❌',
					Iv: '❌ LINK INVÁLIDO ❌'
				},
				only: {
					group: '❌ESSE COMANDO SO PODE SER USADO EM GRUPOS! ❌',
					ownerG: '❌ ESSE COMANDO SO PODE SER USADO PELO DONO DO GRUPO! ❌',
					ownerB: '❌ ESSE COMANDO SO PODE SER USADO PELO MEU DONO! ❌',
					admin: '❌ SILÊNCIO MEMBRO COMUM, VC NÃO TEM MORAL PRA DAR ORDENS AQUI!  ❌',
					Badmin: '❌ O BOT PRECISA SER ADM DO GRUPO!  ❌'
				}
			}
			...
			switch(command) {
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país, senhor ')
					try {
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Desativar com sucesso o modo simih no grupo ✔️')
					} else {
...
//COMANDOS DE PESQUISAS
 ...
 //@Kratos 
				...
				case 'pinterest':
					...
					break 
//FIM DOS COMANDOS, TODAS CASES FORAM MONTADA POR KRATOSÂ©
//PACK V2 VAI SAIR SER VIDEO PEGA 1K DE VIWS

//PACK DE COMANDOS BY @Kratos æ„›
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(mess.wait)
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
